...
//    PdfStreamedDocument document( "tests/tmp.pdf" );
  ...
  Image inputJPG("tests/input.jpg");
  ...
  {
    std::ifstream iccFile("data/ISOnewspaper_v4_26_bas.ICC", std::ios::in | std::ios::binary );
  if( !iccFile.is_open() )
  {
      std::cout << "THere. not there.\n";
     return -1;
  }
  iccFile.seekg( 0, std::ios::end );
  filesizebrg = iccFile.tellg();
  iccFile.seekg( 0, std::ios::beg );
  databrg = new char[filesizebrg];
  }
  
  char *data;
  size_t filesize;
  {
  std::ifstream iccFile("data/sRGB_linear.icc", std::ios::in | std::ios::binary );
  if( !iccFile.is_open() )
  {
      std::cout << "THere. not there.\n";
     return -1;
  ...
  hInProfile = cmsOpenProfileFromFile("data/sRGB_linear.icc", "r"); //sRGB_linear
  hOutProfile = cmsOpenProfileFromFile("data/FakeRBG.icc", "r");
  ...
  PixelPacket *pixels = inputJPG.getPixels(0,0, inputJPG.columns(), inputJPG.rows()); 
  
  unsigned char *transformedRow = new unsigned char[51*inputJPG.columns()];
  
  for (uint32_t row = 0; row < inputJPG.rows(); row++)
  {

      // for now shitty conversion.
       
	for (unsigned int col = 0; col < inputJPG.columns(); col++)
	{
	    PixelPacket *p = &pixels[col + row*inputJPG.columns()];
	    
	    transformedRow[3*col + 0] = (unsigned char) p->red;
	    transformedRow[3*col + 1] = (unsigned char) p->green;
	    transformedRow[3*col + 2] = (unsigned char) p->blue;
	}
      
	cmsDoTransform(hTransform, (unsigned char*) transformedRow, (unsigned char*) transformedRow, 3*inputJPG.columns());
	
	for (unsigned int col = 0; col < inputJPG.columns(); col++)
	{
	    PixelPacket *p = &pixels[col + row*inputJPG.columns()];
	    
	    p->red = transformedRow[3*col + 0];
	    p->green = transformedRow[3*col + 1];
	    p->blue = transformedRow[3*col + 2];
	}
...
*/
  ...
  image_info=MagickCore::CloneImageInfo((MagickCore::ImageInfo *) NULL);
    (void) strcpy(image_info->filename,"tests/input.jpg");
    images=MagickCore::ReadImage(image_info,exception);

    if (exception->severity != MagickCore::UndefinedException)
    ...
    */
    ...
    {
      MagickCore::ProfileImage (image, "icc", data, filesize, MagickFalse);
      MagickCore::ProfileImage (image, "icc", databrg, filesizebrg, MagickFalse);
      imageiccColorProfile
      //MagickCore::SetAdobeRGB1998ImageProfile (image, exception);
    ...
    */
    (void) strcpy(thumbnails->filename,"tests/thumbnail.jpg");
...
*/  
  ...
  MagickCore::ImageInfo *imInfo;
  imInfo = MagickCore::CloneImageInfo((MagickCore::ImageInfo *) NULL);
  (void) strcpy(imInfo->filename, "tests/input.jpg");

  MagickCore::ExceptionInfo exInfo;
  MagickCore::Image *image = MagickCore::ReadImage (imInfo, &exInfo);
  MagickCore::ProfileImage (image, "ICC", data, filesize, MagickFalse);
  
std::cout << "Here.";
//
    (void) strcpy(imInfo->filename,"tests/output.jpg");
    MagickCore::WriteImage(imInfo,image);  
    
//  inputJPG.write("tests/output.jpg");
*/
